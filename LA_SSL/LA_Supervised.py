...
# # ------------------ Resume Setup ------------------ #
# start_epoch = 11  # <<< Set manually to resume from a specific epoch
# best_dice = 0
# checkpoint_path = os.path.join(args.save_path, "last_checkpoint.pth")
...
#     avg_loss = running_loss / len(dataloader)
#     dice_score = -1  # default if not evaluated
...
#     # --------------- Store Epoch Metrics --------------- #
#     metrics.append({'epoch': epoch + 1, 'loss': avg_loss, 'dice': dice_score})

# # ------------------ Save Excel ------------------ #
# df = pd.DataFrame(metrics)
# excel_path = os.path.join(args.save_path, 'metrics.xlsx')
# df.to_excel(excel_path, index=False)
# logging.info(f"Saved training metrics to {excel_path}")
# logging.info(f"Training complete. Best Dice Score: {best_dice:.4f}")
...
# ------------------ Resume Setup ------------------ #
start_epoch = 10  # <<< CHANGE HERE to resume training
best_dice = 0
checkpoint_path = os.path.join(args.save_path, "last_checkpoint.pth")
