...
class DownsamplingConvBlock(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        super(DownsamplingConvBlock, self).__init__()
        ops = [
            nn.Conv3d(n_filters_in, n_filters_out, kernel_size=2, stride=stride, padding=0)
        ]
        if normalization == 'batchnorm':
            ops.append(nn.BatchNorm3d(n_filters_out))
        elif normalization == 'groupnorm':
...
class UpsamplingDeconvBlock(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        super(UpsamplingDeconvBlock, self).__init__()
        ops = [
            nn.ConvTranspose3d(n_filters_in, n_filters_out, kernel_size=2, stride=stride, padding=0)
        ]
        if normalization == 'batchnorm':
            ops.append(nn.BatchNorm3d(n_filters_out))
        elif normalization == 'groupnorm':
...
class VNet(nn.Module):
    def __init__(self, n_channels=1, n_classes=2, n_filters=16, normalization='instancenorm', has_dropout=False):
        ...
        self.branchs = nn.ModuleList()
        for _ in range(1):
            layers = [ConvBlock(1, n_filters, n_filters, normalization)]
            if has_dropout:
                layers.append(nn.Dropout3d(p=0.5))
            layers.append(nn.Conv3d(n_filters, n_classes, kernel_size=1, padding=0))
    ...
    def decoder(self, features):
        ...
        outputs.append(features[3])  # x6 (skip connection feature for auxiliary use)

        return outputs

    def forward(self, x, turnoff_drop=False):
        if turnoff_drop:
            old_dropout = self.has_dropout
            self.has_dropout = False
